...
class ThesisTestCase(APITestCase):
    ...
    def setUp(self) -> None:
        self.url = reverse('thesis:thesis-list')
        ...
        res = self.client.post(reverse('accounts:login'), data)
        self.access_token = res.data['access']

    @patch('riiid_django.utils.openai.OpenAI.get_answer', return_value='OpenAI Result Mock')
    @patch('accounts.models.User.get_cached_thesis_by_url', return_value=None)
    def test_논문요약_요청_성공(self, mocked_redis_call, mock_openai_call):
        """논문 요약 요청, Redis 가져오는부분 mocking 처리"""
        # Given
        url = reverse('thesis:thesis-list')
